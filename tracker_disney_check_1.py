...
def compare_trajectory_with_logo(trajectory, logo):
    ...
    # Calcular la correlación entre la trayectoria y el logo utilizando matchTemplate
    result = cv2.matchTemplate(scaled_trajectory_gray, logo_gray, cv2.TM_CCOEFF_NORMED)
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)

    return max_val


def initialize_tracker(picam):
    ...
    while True:
        ...
        # Comprobar si la trayectoria coincide con el logo
        similarity = compare_trajectory_with_logo(trajectory, disney_logo)
